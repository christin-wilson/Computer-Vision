...
int main()
  ...
  {
  FILE		*fpt1,*fpt2;
  unsigned char	*image,*MSF,*temp,*check,*temp1;
  int threshold=0;
  unsigned char	*final;
  char		header[320];
  char    header2[320];
  ...
  temp1=temp=(unsigned char *)calloc(17*11,sizeof(unsigned char));
  temp=(unsigned char *)calloc(15*9,sizeof(unsigned char));
  check=(unsigned char *)calloc(17*11,sizeof(unsigned char));
    ...
    int MARKED=1;  
      ...
      if (ob==1)
      {
        ...
        for(x=1;x<ROWS+1;x++)
        {
          for(y=1;y<COLS2+1;y++)
          {
            temp1[x*(COLS2+2)]=temp[(x-1)*COLS2+(y-1)];
          }  
        }
        while(MARKED!=0)
        {
          ...
          for(x=1;x<ROWS2+1;x++)
          {
            ...
            {
              check[x*(COLS2+2)+y]=0;
              if(temp1[x*COLS2+2+y]==0)
              { 
                transitions=0;

                if((temp1[(x-1)*(COLS2+2)+(y-1)])==0 && (temp1[(x-1)*(COLS2+2)+y])==255)
                {  transitions++;     }  
            
                if((temp1[(x-1)*(COLS2+2)+y])==0 && (temp1[(x-1)*(COLS2+2)+(y+1)])==255)
                {  transitions++;     }
 
                if((temp1[(x-1)*(COLS2+2)+(y+1)])==0 && (temp1[(x)*(COLS2+2)+(y+1)])==255)
                {  transitions++;     }

                if((temp1[(x)*(COLS2+2)+(y+1)])==0 && (temp1[(x+1)*(COLS2+2)+(y+1)])==255)
                {  transitions++;     }
                ...
                if((temp1[(x-1)*(COLS2+2)+(y-1)])==0)
                {  neighbours++;     }  
            
                if((temp1[(x-1)*(COLS2+2)+y])==0)
                {  neighbours++;     }
 
                if((temp1[(x-1)*(COLS2+2)+(y+1)])==0)
                {  neighbours++;     }

                if((temp1[(x)*(COLS2+2)+(y+1)])==0)
                {  neighbours++;     }
                ...
                if((temp1[(x-1)*(COLS2+2)+(y)])==255)
                {  edgecheck=1;     }

                else if((temp1[(x)*(COLS2+2)+(y+1)])==255)
                {  edgecheck=1;     }

                else if((temp1[(x+1)*(COLS2+2)+(y)])==255 && (temp1[(x)*(COLS2+2)+(y-1)])==255)
                {  edgecheck=1;     }
          ...
          for(x=1;x<ROWS2+1;x++)
          {
            for(y=1;y<COLS2+1;y++)
            {
              if(check[x*(COLS2+2)+y]==1)
              {
              temp1[x*(COLS2+2)+y]=255;
              }
            }
          }    
        ...
        for(x=1;x<ROWS2+1;x++)
        {
          ...
          { 
            ...
            if(temp[x*(COLS2+2)+y]==0)
            {               
              check[x*(COLS2+2)+y]=0;
              transitions=0;

                if((temp[(x-1)*(COLS2+2)+(y-1)])==0 && (temp[(x-1)*(COLS2+2)+y])==255)
                {  transitions++;     }  
            
                if((temp[(x-1)*(COLS2+2)+y])==0 && (temp[(x-1)*(COLS2+2)+(y+1)])==255)
                {  transitions++;     }
 
                if((temp[(x-1)*(COLS2+2)+(y+1)])==0 && (temp[(x)*(COLS2+2)+(y+1)])==255)
                {  transitions++;     }

                if((temp[(x)*(COLS2+2)+(y+1)])==0 && (temp[(x+1)*(COLS2+2)+(y+1)])==255)
                {  transitions++;     }
        ...
        for(x=1;x<ROWS2+1;x++)
          {
            for(y=1;y<COLS2+1;y++)
            {
              if(check[x*(COLS2+2)+y]==1)
              {
              temp1[x*(COLS2+2)+y]=180;
              }
              else if(check[x*(COLS2+2)+y]==2)
              {
              temp1[x*(COLS2+2)+y]=90;
              }
            }
          }
